...
use db_joins ;
CREATE TABLE db_joins.employees(
...
INSERT INTO db_joins.employees
VALUES
(1, 'John','Kelly', 101 , 201 , 60000.00),
(2, 'Jane', 'Smith', 102, 202, 75000.00),
(3, 'Mike', 'Brown', 101, 203, 50000.00),
(4, 'Emily', 'Davis', 103, 204, 80000.00),
(5, 'Robert', 'Wilson', 102, 205, 70000.00),
(6, 'Jessica', 'Taylor', 101, 201, 55000.00),
(7, 'Paul', 'Anderson', 104, 206, 90000.00),
(8, 'Laura', 'Thomas', 105, 207, 65000.00)
;

Select * from db_joins.employees;

CREATE TABLE db_joins.departments (
...
INSERT INTO db_joins.departments

VALUES
(101, 'Human Resources', 'New York', 1),
(102, 'Finance', 'London', 2),
(105, 'Engineering', 'San Francisco', 4) ;

select * from db_joins.departments;

CREATE TABLE db_joins.projects (
...
INSERT INTO db_joins.projects (project_id, project_name, start_date, end_date, budget) VALUES
(201, 'Project Alpha', '2023-01-01', '2023-12-31', 100000.00),
(202, 'Project Beta', '2023-02-01', '2023-11-30', 120000.00),
(207, 'Project Gamma', '2023-03-01', '2023-10-31', 90000.00) ;

select * from db_joins.projects;


Select *
from db_joins.employees LEFT JOIN db_joins.departments
on db_joins.employees.dept_id = db_joins.departments.dept_id;
...
-- RIGHT JOIN
use db_joins ; #set db as default
SELECT * 
FROM employees RIGHT JOIN departments
ON employees.dept_id = departments.dept_id ;

--  use alias for tbl name

SELECT * 
FROM employees as t1 RIGHT JOIN departments t2
ON t1.dept_id = t2.dept_id ;

-- inner join, choose columns
# employee details with project
SELECT * 
FROM employees as t1 INNER JOIN projects AS t2
...
SELECT 
	...
	first_name
FROM
	employees AS t1 INNER JOIN projects AS t2 
    ON t1.project_id = t2.project_id;

/*
-- FULL OUTER JOIN -- UNION ALL (KEEPS THE DUPS) ; UNION (REMOVES DUPS) 
APPEND tbls (left + right) together
*/
...
UNION ALL
SELECT *
FROM db_joins.employees RIGHT JOIN db_joins.departments
ON db_joins.employees.dept_id = db_joins.departments.dept_id ;
...
CREATE DATABASE db_norm ;

CREATE TABLE db_norm.employees (
	emp_id CHAR(5) PRIMARY KEY, # implies not null and unique
...
Select *
from db_norm.employees;

describe db_norm.employees;

SELECT * FROM db_norm.employees;

DESCRIBE db_norm.employees;  # structure of the table

 
INSERT INTO db_norm.employees
VALUES
('A1234', 'D1' , 'John','Kramer', '1995-09-08' ) ;
SELECT * FROM db_norm.employees;

INSERT INTO db_norm.employees
VALUES
('A1234', 'D1' , 'John','Kramer', '1995-09-08' ),
('A1235', 'D1', 'Rakesh', 'Kumar', '04-05-1998') ;

/*
Error Code: 1062. Duplicate entry 'A1234' for key 'employees.PRIMARY'
*/

INSERT INTO db_norm.employees
VALUES
('A1236', 'D1' , 'Jane','Austin', '1990-01-08' ),
('A1235', 'D2', 'Rakesh', 'Kumar', '1998-04-05'),
('A1238', 'D3', 'Nishant', 'Sharma', '2003-07-27') ;
SELECT * FROM db_norm.employees;

/*
2 row(s) affected, 1 warning(s): 1265 
Data truncated for column 'DOB' at row 2 Records: 2  Duplicates: 0  Warnings: 1
*/

INSERT INTO db_norm.employees
VALUES
('A1238', 'Aditi', 'Naik', '1997-09-04') ;
...
INSERT INTO db_norm.employees
VALUES
('A1238',null, 'Aditi', 'Naik', '1997-09-04') ;

/*
Error Code: 1048. Column 'dept_name' cannot be null
*/

INSERT INTO db_norm.employees
VALUES
('A1240','NA', 'Aditi', 'Naik', '1997-09-04') ;

SELECT * FROM db_norm.employees;

INSERT INTO db_norm.employees
VALUES
('A1238','D5', 'Sofie', '1997-09-04') ;

/*
Error Code: 1136. Column count doesn't match value count at row 1
*/

INSERT INTO db_norm.employees(emp_id,dept_name,First_name,DOB)
VALUES
('A1239','D5', 'Sofie', '1997-09-04') ;

INSERT INTO db_norm.employees(emp_id,First_name,DOB)
VALUES
('A1240', 'Sofie', '1997-09-04') ;

/*
1 row(s) affected, 1 warning(s): 1364 
Field 'dept_name' doesn't have a default value
*/

CREATE TABLE db_norm.Sales_data (
...
insert into db_norm.Sales_data
VALUES
(123, 32, 'A1222'),
(124, 43, 'A1234'),
(125, 2, 'A1234')
...
a foreign key constraint fails (`db_norm`.`sales_data`, 
CONSTRAINT `sales_data_ibfk_1` FOREIGN KEY (`emp_id`) 
...
SELECT * FROM db_norm.sales_data;

/* 
Composite key:when a combination of multiple fields together are used as a primary key 
*/

CREATE TABLE db_norm.emp_bank_details(
	emp_id char(5) NOT NULL,
	emp_acnt varchar(50) NOT NULL,
	emp_JoinDate DATE not null,
	emp_bank_name varchar(50) not null,
	emp_branch_name varchar(50) not null,
	PRIMARY KEY(emp_id,emp_acnt), # composite key
	FOREIGN KEY(emp_id) REFERENCES employees(emp_id)
    ) ;

DESCRIBE db_norm.emp_bank_details ;
DESCRIBE db_norm.sales_data ;

 /*
- Default : Sets the value for a column if no value is provided.
- Check : Conditional expression which validates the entered 
*/

CREATE TABLE db_norm.emp_hiring_1(
	app_id INT NOT NULL,
	Cand_name varchar(50) NOT NULL,
	Age INT CHECK (Age>=18 AND Age<=65),
	Exp_Sal INT CHECK (Exp_Sal>0),
	int_date DATE NOT NULL DEFAULT '2025-08-30'
);

INSERT INTO db_norm.emp_hiring_1(app_id,Cand_name,Age)
VALUES
(1,'Abc', 34),
(2,'cv', 32),
(3,'res', 12) ;
...
/*
Error Code: 3819. Check constraint 'emp_hiring_1_chk_1' is violated.
*/
...
SELECT * FROM db_norm.emp_hiring_1;

CREATE TABLE db_norm.emp_hiring_2(
...
INSERT INTO db_norm.emp_hiring_2(Cand_name,Age)
VALUES
('WER', 32),
('ABC', 22) ;

SELECT * FROM db_norm.emp_hiring_2;

CREATE TABLE db_norm.emp_hiring_3(
	app_id INT PRIMARY KEY AUTO_INCREMENT,
	Cand_name varchar(50) NOT NULL,
	Age INT CHECK (Age>=18 AND Age<=65),
    Exp_Sal INT CHECK (Exp_Sal>0),
...
ALTER TABLE db_norm.emp_hiring_3
AUTO_INCREMENT = 100 ;
SET @@auto_increment_increment = 5 ;
DESCRIBE db_norm.emp_hiring_3;
-- change the start value, BEFORE INSERT VALUE
INSERT INTO db_norm.emp_hiring_3(Cand_name,Age)
VALUES
('WER', 32),
('ABC', 22),
('JAN', 44) ;
SELECT * FROM db_norm.emp_hiring_3;

create DATABASE db_joins1 ;
  create table db_joins1.employees(
   emp_id char(4),
   emp_name varchar(50),
   emp_age int,
   emp_dept varchar(20),
   emp_sal float);
  insert into db_joins1.employees
  values
  ...
  ;  
  SELECT * FROM db_joins1.employees;
  USE db_joins1 ;
  
select emp_dept, avg(emp_sal) as 'avg_sal'
from employees
group by emp_dept; # dept wise avg

/*
exp(colname/blank) OVER( PARTITION BY col1, col2 ORDER BY col1, 
col2 acs/desc)
*/
-- Overall Avg
SELECT * , avg(emp_sal) OVER() AS 'avg1'
FROM db_joins1.employees;

-- avg by each dept
SELECT * , avg(emp_sal) OVER() AS 'avg1',
avg(emp_sal) OVER( PARTITION BY emp_dept) AS 'avg_by_dept'
FROM db_joins1.employees;
...
SELECT *,
ROW_NUMBER() OVER() as 'Rownnum1',
ROW_NUMBER() OVER( ORDER BY emp_sal DESC ) as 'RowasRank'
FROM db_joins1.employees;

SELECT *,
RANK() OVER() as 'Rank1',
RANK() OVER( ORDER BY emp_sal DESC ) as 'Rank2'
FROM db_joins1.employees;

SELECT *,
dense_rank() Over() as 'DenRank1', 
DENSE_RANK() OVER (ORDER BY emp_sal DESC) AS 'DenRank2',
dense_rank() OVER(partition by emp_dept order by emp_sal DESC) as 'DenseRank'
FROM
db_joins1.employees;

-- Running total
SELECT *,
SUM(emp_sal) OVER( ORDER BY emp_sal DESC ) as 'Running_Total'
FROM db_joins1.employees;

SELECT *,
SUM(emp_sal) OVER( ORDER BY emp_sal DESC Rows between unbounded preceding and current row) as 'Running_Total'
FROM db_joins1.employees;
...
SELECT orderNumber, 
SUM(quantityOrdered*priceEach) AS 'total order value'
FROM orderdetails 
GROUP BY orderNumber ;

SELECT 
    o.orderNumber, od.total_order_value, o.status
FROM
    orders o
        JOIN
    (SELECT 
...
# ORDERDETAILS 
-- PRODUCT CODE OF ALL THE PRODUCTS WHICH HAVE QTYORDERED < 20 
--  prod_name
...
FROM
    products
WHERE
    productCode IN (SELECT DISTINCT
            productCode
        FROM
            orderdetails
        WHERE
...
WITH CTE(subquery)_NAME (SELECT COLS, CALC, FROM, GROUPBY..)
main query - select col1,cl2 FROM CTE_NAME joins table1, groupby ;
*/
/*Example
to calculate the total order value for each order with Status.
*/

-- Non Recursive (Simple Query)
WITH orderValue as (select orderNumber, 
sum(quantityOrdered*priceEach) as 'total_order_value'
FROM orderdetails 
group by orderNumber )
SELECT o.orderNumber , orderValue.total_order_value, o.status
FROM orders o INNER JOIN orderValue 
ON o.orderNumber = orderValue.orderNumber
ORDER BY o.status ;

/*
-- recursive (queries that refer to themselves).
...
*/
/* Example
traverse a hierarchical employee structure,
...
WITH RECURSIVE EmployeeHierarchy AS (
    ...
    FROM employees
    WHERE reportsTo IS NULL   -- Top managers
    
    UNION ALL
    
    SELECT e.employeeNumber, e.firstName, e.reportsTo
    FROM employees e
...
/*
FISCAL Data 
year wise month wise total order value
YEAR | MONTH# | MONTH TOTALorder value | YEAR_TOTALorder value |,.....
1. orderdetails - Order_id | sum(quantityOrdered*priceEach) 
2. orders -   YEAR(date) | Order_id | sum(quantityOrdered*priceEach)
3. joined -  extract year, month, group by year, month|sum(quantityOrdered*priceEach)
4. windows function
*/
-- WITH CTE
WITH order_value AS ( SELECT 
    ...
    FROM
        orderdetails
    GROUP BY 1),
order_val_data as ( select  o.orderNumber, o.orderDate, 
total_order_value
    FROM
orders o, order_value where o.orderNumber = order_value.orderNumber),
fiscal_data as (SELECT 
    YEAR(orderDate) AS fiscal_year,
	MONTHNAME(orderdate) AS month_of_year,
	SUM(total_order_value) AS total_order_value
FROM order_val_data GROUP BY 1,2)
SELECT *,
SUM(total_order_value) OVER(partition by fiscal_year) AS year_total,
SUM(total_order_value) OVER(partition by fiscal_year,month_of_year)
AS Year_Mon_total
from fiscal_data;


/* A stored procedure is a set of statements that are 
saved in the database and can be executed as a single unit.
Stored procedures allow for modular programming, reusability, 
and better performance, especially when running complex operations 
repeatedly.
*/
# share customers data with each country separately
-- 27 countries
Select * from customers where country = 'usa' ;
/*
# reset the delmiter //
DELIMITER //
CREATE PROCEDURE <NAMEOF PROCEDURE>
BEGIN
<PROCESS==QUERY/IES,LOOPS,MORE,CTES,WIN,OPS> ;
END //
DELIMITER ;
reset it back to default ';'
*/
DELIMITER //
CREATE PROCEDURE GetCusotmerUSA()
BEGIN
Select * from customers where country = 'usa' ;
END //
DELIMITER ;
...
-- Stored procedure with Inputs, outputs variables
DELIMITER //
CREATE PROCEDURE GetCustomerdata(IN cntry_in VARCHAR(50))
BEGIN
Select customerNumber,customerName,country
 from customers where country = cntry_in ;
END //
DELIMITER ;
CALL GetCustomerdata('france');
CALL GetCustomerdata('germany');